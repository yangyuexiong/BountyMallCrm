  ...
  methods: {
    ...
    handleClose(index) {
      setTimeout(() => {
        ...
        const needDelete = !(
          closePath === "/home/index" && this.tagList.length === 1
        );
    ...
    closeAll() {
      ...
      this.tagList.map((e, i) =>
        this.needAskUrls.includes(e.path) ? indexArr.push(i) : ""
      );
    ...
    closeOther() {
      const indexArr = [];
      this.tagList.map((e, i) =>
        this.needAskUrls.includes(e.path) && e.fullPath !== this.$route.path
          ? indexArr.push(i)
          : ""
      );
      if (indexArr.length > 0) {
